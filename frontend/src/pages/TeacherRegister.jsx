...
const TeacherRegister = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await register({ name, username, password }, 'teacher');
      navigate('/');
    } catch (err) {
    ...
    <div className="min-h-screen bg-slate-950 flex flex-col justify-center py-12 sm:px-6 lg:px-8 font-sans">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <p className="mt-2 text-center text-sm text-slate-500 font-semibold uppercase tracking-wider">
          Educator Platform Registration
        </p>
      </div>

      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-slate-900 py-8 px-4 border border-slate-800 shadow-xl rounded-2xl sm:px-10">
          <h3 className="text-lg font-bold text-slate-100 mb-6">Create Educator Account</h3>
          ...
          <div className="mt-6 border-t border-slate-800 pt-4 text-center">
            <p className="text-xs text-slate-500">
              ...
              <Link to="/teacher/login" className="font-bold text-indigo-400 hover:text-indigo-300">
                Educator Login
              </Link>
            </p>
          </div>
